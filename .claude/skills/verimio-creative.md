...
|------|-----|-----------|-----------------|
| **Midnight** | `#0F172A` | `--primary` | Ana marka rengi. Logo zemini, dark section bg, CTA arka plan varyantı |
| **Soft Blue** | `#60A5FA` | `--primary-light` | İkincil aksanlar, icon rengi, border-accent, gradient başlangıcı |
| **Amber** | `#F59E0B` | `--secondary` | **Ana CTA rengi.** Sadece aktif/tamamlanan/sonuç noktalarında. Dominant değil — aksan. |
| **Amber Hover** | `#FBBF24` | `--secondary-hover` | Lime hover state |
| **Slate-800** | `#020617` | `--dark` | Dark section overlay, en derin karanlık |
...
**Kritik kural — Amber aksanı:** `#F59E0B` yalnızca "başarı / tamamlanma / en önemli nokta" anlamına gelir. Her yerde kullanılmaz. Nadir kullanım gücünü artırır.

**Gradient text:** `linear-gradient(135deg, #60A5FA 0%, #F59E0B 100%)` — başlıklarda vurgulu kelimeler için. CSS class: `.gradient-text`
...
**Wordmark:** `verim` (ana renk) + `io` (her zaman `#F59E0B` Amber)
**Monogram (SVG):** `#0F172A` kare zemin (rx=14) + beyaz V path + `#F59E0B` küçük daire (V'nin üst noktasında)
...
```
BRAND_STYLE.light = "cream white warm background, centered soft midnight radial gradient glow,
subtle geometric dot grid pattern in soft blue fading outward from center,
three small amber (#F59E0B) circle accents asymmetrically placed,
...
```
BRAND_STYLE.dark = "deep rich midnight to soft blue gradient background,
centered glowing halo light effect, subtle geometric dot grid in lighter blue fading outward,
three small amber (#F59E0B) accent marks,
...
- `#60A5FA` (Soft Blue) → yapısal elemanlar (çizgiler, node'lar, şekiller)
- `#F59E0B` (Amber) → **sadece** çıktı/sonuç/aktif nokta
...
**Evrensel ikon kuralı:** `currentColor` ana yapı, `#F59E0B` (Amber) yalnızca tamamlanmış/aktif nokta.
...
- **Kural 2:** Amber az, midnight çok. Amber sadece vurgu.
...
**Ne kullanma:** Neon renkleri, aşırı animasyon, 3D render figürler, stock fotoğraf görünümü, eski indigo/lime/purple palet.
...
- Renk token'ları: `--primary` (#020617 Midnight), `--surface` (#0F172A Slate-900), `--secondary` (#F59E0B Amber), `--support` (#60A5FA Soft Blue) ve semantic varyantları
...
- Dark mode karşılıkları: admin panel renk paleti (#020617 bg) ile ana site (#FAFAF9 bg) arasındaki geçiş mantığı
...
```
RENK KONTRAST:
□ #F59E0B (amber) on #020617 (midnight): yüksek kontrast ✅
□ #60A5FA (soft blue) on #FAFAF9 (off-white): kontrast kontrol gerekli
□ #94A3B8 (muted text) on #020617: ✅
□ Amber CTA button: amber bg + midnight text → kontrol gerekli
