...
const COLORS = {
  core: [
    { name: 'Deep Indigo', token: 'primary', hex: '#0F172A', use: 'Ana arka plan, navbar', dark: true },
    { name: 'Vivid Lime', token: 'secondary', hex: '#F59E0B', use: 'CTA, vurgu, logo io', dark: false },
    { name: 'Dark Indigo', token: 'dark', hex: '#020617', use: 'Card arka planı', dark: true },
    { name: 'Soft Purple', token: 'primary-light', hex: '#F59E0B', use: 'Gradient, hover, link', dark: true },
  ],
  neutral: [
  ...
  extended: [
    { name: 'Lime 300', token: 'lime-300', hex: '#FBBF24', use: 'Hover state, soft vurgu', dark: false },
...
// ─── Logo Varyasyonları ───────────────────────────────────────────────────────
const LOGO_VARIANTS = [
  { id: 'dark-full', label: 'Dark — Full Wordmark', bg: '#0F172A', verim: '#FFFFFF', io: '#F59E0B', size: 52 },
  { id: 'light-full', label: 'Light — Full Wordmark', bg: '#F5F5F4', verim: '#0F172A', io: '#4D7C0F', size: 52 },
  { id: 'dark-mono', label: 'Dark — Monochrome', bg: '#0F172A', verim: '#FFFFFF', io: '#FFFFFF', size: 52 },
  { id: 'light-mono', label: 'Light — Monochrome', bg: '#F5F5F4', verim: '#1C1917', io: '#1C1917', size: 52 },
  { id: 'dark-reverse', label: 'Dark — Lime Heavy', bg: '#020617', verim: '#F59E0B', io: '#FFFFFF', size: 52 },
  { id: 'black-full', label: 'Black — Full', bg: '#000000', verim: '#FFFFFF', io: '#F59E0B', size: 52 },
]

const MONOGRAM_VARIANTS = [
  { id: 'v-dark', label: 'Dark', bg: '#0F172A', v: '#FFFFFF', dot: '#F59E0B' },
  { id: 'v-light', label: 'Light', bg: '#F5F5F4', v: '#0F172A', dot: '#4D7C0F' },
  { id: 'v-lime', label: 'Lime BG', bg: '#F59E0B', v: '#0F172A', dot: '#020617' },
  { id: 'v-black', label: 'Black', bg: '#000000', v: '#FFFFFF', dot: '#F59E0B' },
]
...
const FONT_SAMPLES = [
  ...
  { label: 'Mono — 14px', size: 14, weight: 400, text: '#0F172A  #F59E0B  font-weight: 700', mono: true },
]

const SOCIAL_PROFILES = [
  { id: 'linkedin', label: 'LinkedIn Profil', size: '400×400', bg: '#0F172A', v: '#FFFFFF', dot: '#F59E0B', border: '#F59E0B' },
  { id: 'twitter', label: 'Twitter/X Profil', size: '400×400', bg: '#000000', v: '#FFFFFF', dot: '#F59E0B', border: '#1A1A1A' },
  { id: 'instagram', label: 'Instagram Profil', size: '400×400', bg: '#020617', v: '#FFFFFF', dot: '#F59E0B', border: '#F59E0B' },
  { id: 'whatsapp', label: 'WhatsApp / iMessage', size: '400×400', bg: '#F59E0B', v: '#0F172A', dot: '#020617', border: '#F59E0B' },
]
  ...
  illustration: [
    {
      ...
      prompt: "Abstract geometric corporate technology illustration, dark deep indigo background (#020617), interconnected nodes and data flow lines in soft purple, a structured bottleneck filter transforming chaotic inputs into clean parallel outputs, three rising bar chart columns at bottom with the rightmost bar in vivid lime green (#F59E0B), minimal flat vector art style, no text, no people, technical precision",
...
const label = (text: string, sub?: string) => (
  <div style={{ background: '#0F172A', borderTop: '1px solid #0F172A', padding: '10px 14px' }}>
...
// ─── İkon kutusu ─────────────────────────────────────────────────────────────
function IconBox({ Icon, name, sub, size = 24, bg = '#0F172A' }: {
...
export default function BrandPreviewClient() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0514', fontFamily: 'var(--font-dm-sans, system-ui)', color: '#FFFFFF' }}>
      ...
      {/* Header */}
      <div style={{ borderBottom: '1px solid #0F172A', padding: '16px 32px', display: 'flex', flexDirection: 'column', gap: 12, position: 'sticky', top: 0, background: '#0A0514', zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: '#F59E0B', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ color: '#0F172A', fontWeight: 900, fontSize: 16 }}>V</span>
          </div>
          <div>
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                ...
                fontSize: 13,
                fontWeight: 600,
                background: section === t.id ? '#F59E0B' : '#0F172A',
                color: section === t.id ? '#0F172A' : '#78716C',
      ...
      <div style={{ padding: '40px' }}>
        ...
        {section === 'logo' && (
          <div>
            ...
            <div style={{ background: '#0F172A', borderRadius: 16, padding: '40px 48px', border: '1px solid #0F172A', display: 'flex', alignItems: 'center', gap: 48, flexWrap: 'wrap', marginBottom: 48 }}>
              {[16, 24, 32, 48, 72, 96].map((size) => (
                <div key={size} style={{ textAlign: 'center' }}>
                  <div style={{ fontWeight: 700, fontSize: size, letterSpacing: '-0.02em', lineHeight: 1 }}>
                    ...
                    <span style={{ color: '#F59E0B' }}>io</span>
                  </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              {SOCIAL_PROFILES.map((p) => (
                <div key={p.id} style={card()}>
                  <div style={{ background: '#0F172A', padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        {section === 'colors' && (
          <div>
            {Object.entries(COLORS).map(([group, colors]) => (
              <div key={group} style={{ marginBottom: 40 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                  {colors.map((c) => (
                    <div key={c.token} onClick={() => copy(c.hex)} style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid #0F172A', cursor: 'pointer' }}>
                      ...
                      <div style={{ background: '#0F172A', padding: '12px 14px' }}>
                        ...
                        <div style={{ fontSize: 12, color: '#F59E0B', fontFamily: 'monospace' }}>{copied === c.hex ? '✓ Kopyalandı' : c.hex}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
              {[
                { label: 'Indigo → Lime', bg: 'linear-gradient(135deg, #0F172A 0%, #F59E0B 100%)' },
                { label: 'Purple → Lime', bg: 'linear-gradient(135deg, #F59E0B 0%, #F59E0B 100%)' },
                { label: 'Dark → Indigo', bg: 'linear-gradient(135deg, #000000 0%, #0F172A 100%)' },
              ].map((g) => (
                <div key={g.label} style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid #0F172A' }}>
                  <div style={{ background: g.bg, height: 80 }} />
                  <div style={{ background: '#0F172A', padding: '10px 14px', fontSize: 12, color: '#78716C' }}>{g.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {section === 'typography' && (
          <div>
            ...
            <div style={{ background: '#0F172A', borderRadius: 16, padding: 32, border: '1px solid #0F172A', marginBottom: 24 }}>
            ...
            <div style={{ background: '#0F172A', borderRadius: 16, padding: 32, border: '1px solid #0F172A', marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {FONT_SAMPLES.map((s) => (
                  <div key={s.label} style={{ borderBottom: '1px solid #0F172A', paddingBottom: 16 }}>
                    <div style={{ fontSize: 11, color: '#F59E0B', marginBottom: 6, fontFamily: 'monospace' }}>{s.label}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={{ background: '#0F172A', borderRadius: 16, padding: 32, border: '1px solid #0F172A' }}>
                <div style={{ fontSize: 11, color: '#F59E0B', marginBottom: 16, letterSpacing: '0.1em' }}>DARK MODE</div>
                ...
                <button style={{ background: '#F59E0B', color: '#0F172A', border: 'none', padding: '12px 24px', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Check-Up Başlatın</button>
              </div>
              <div style={{ background: '#F5F5F4', borderRadius: 16, padding: 32, border: '1px solid #E5E5E3' }}>
                ...
                <div style={{ fontSize: 36, fontWeight: 700, color: '#0F172A', marginBottom: 8, lineHeight: 1.2 }}>İş Süreçlerini<br />AI ile Dönüştür</div>
                <div style={{ fontSize: 16, color: '#78716C', marginBottom: 20, lineHeight: 1.6 }}>3 dakikada şirketinin AI hazırlık seviyesini öğren.</div>
                <button style={{ background: '#0F172A', color: '#FFFFFF', border: 'none', padding: '12px 24px', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Check-Up Başlatın</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 48 }}>
              {SERVICE_ICONS.map(({ label: lbl, sub, Icon }) => (
                <IconBox key={lbl} Icon={Icon} name={lbl} sub={sub} bg="#0F172A" />
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 480 }}>
              {[
                ...
                <div key={title} style={{ borderRadius: 16, padding: '20px 24px', border: '1px solid #0F172A', background: '#0F172A', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(139,92,246,0.12)', color: '#F59E0B', flexShrink: 0 }}>
                    <Icon size={20} />
                  </div>
                  <div>
        ...
        {section === 'illustration' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 48 }}>
              <div>
                <div style={{ fontSize: 12, color: '#4C4462', marginBottom: 12, fontFamily: 'monospace', letterSpacing: '0.08em' }}>DARK BG (#0F172A)</div>
                <div style={{ background: '#0F172A', borderRadius: 20, padding: 40, border: '1px solid #0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <HakkimizdaHeroIllustration className="max-w-sm" />
                </div>
              </div>
              <div>
                <div style={{ fontSize: 12, color: '#4C4462', marginBottom: 12, fontFamily: 'monospace', letterSpacing: '0.08em' }}>DARK INDIGO (#020617)</div>
                <div style={{ background: '#020617', borderRadius: 20, padding: 40, border: '1px solid #0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <HakkimizdaHeroIllustration className="max-w-sm" />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#0F172A', borderRadius: 20, padding: '40px 48px', border: '1px solid #0F172A', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'center', marginBottom: 48 }}>
              <div>
                <div style={{ fontSize: 11, color: '#F59E0B', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 16 }}>VERİMİO HAKKINDA</div>
                <div style={{ fontSize: 32, fontWeight: 700, color: '#FFFFFF', lineHeight: 1.2, marginBottom: 16 }}>Operasyonel<br /><span style={{ color: '#F59E0B' }}>mükemmelliği</span><br />birlikte inşa edelim.</div>
            ...
            <div style={{ maxWidth: 600 }}>
              <div style={{ background: '#0F172A', borderRadius: 20, padding: 32, border: '1px solid #0F172A' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
              {[
                ...
                <div key={post.title} style={{ background: '#0F172A', borderRadius: 20, border: '1px solid #0F172A', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '20px 20px 24px' }}>
                    ...
                    <div style={{ fontSize: 13, color: '#F59E0B', fontWeight: 600 }}>Devamını oku →</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {section === 'templates' && (
          <div>
                ...
                {(['linkedin', 'instagram', 'twitter', 'story'] as SocialAspect[]).map((a) => (
                  <button
                    ...
                    style={{
                      ...
                      background: socialAspect === a ? '#F59E0B' : '#0F172A',
            ...
            {/* Boyut bilgisi */}
            <div style={{ display: 'inline-flex', gap: 8, alignItems: 'center', background: '#0F172A', borderRadius: 8, padding: '6px 14px', marginBottom: 32 }}>
        ...
        {section === 'social' && (
          <div>
            ...
            <div style={{ marginBottom: 48, borderRadius: 16, overflow: 'hidden', border: '1px solid #0F172A' }}>
              <div style={{ background: '#0F172A', width: '100%', aspectRatio: '1584/396', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 6%', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', right: '-5%', top: '-40%', width: '45%', aspectRatio: '1', borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,158,11,0.12) 0%, transparent 70%)' }} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '2%' }}>
                    <div style={{ width: '5%', aspectRatio: '1', minWidth: 32, background: '#F59E0B', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '3%' }}>
                  ...
                  <div style={{ background: '#F59E0B', color: '#0F172A', padding: '6px 16px', borderRadius: 8, fontSize: 'clamp(8px, 1.2vw, 14px)', fontWeight: 700 }}>Check-Up Başlatın →</div>
                  <div style={{ color: '#4C4462', fontSize: 'clamp(7px, 1vw, 12px)' }}>verimio.com.tr</div>
                </div>
              </div>
              <div style={{ background: '#0F172A', padding: '10px 16px', fontSize: 12, color: '#4C4462' }}>LinkedIn Banner — 1584×396</div>
            </div>
            ...
            <div style={{ marginBottom: 48, borderRadius: 16, overflow: 'hidden', border: '1px solid #0F172A' }}>
              <div style={{ background: '#000000', width: '100%', aspectRatio: '1500/500', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: '60%', aspectRatio: '1', borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,158,11,0.08) 0%, transparent 70%)' }} />
              ...
              <div style={{ background: '#0F172A', padding: '10px 16px', fontSize: 12, color: '#4C4462' }}>Twitter/X Header — 1500×500</div>
            </div>
            ...
            <div style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #0F172A' }}>
              <div style={{ background: '#0F172A', width: '100%', aspectRatio: '1200/630', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'space-between', padding: '6%', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: '-15%', right: '-10%', width: '40%', aspectRatio: '1', borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,158,11,0.15) 0%, transparent 70%)' }} />
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5%' }}>
                  <div style={{ width: '5%', minWidth: 28, aspectRatio: '1', background: '#F59E0B', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <svg viewBox="0 0 64 64" fill="none" style={{ width: '60%', height: '60%' }}>
                      <path d="M8 12 L32 52 L56 12" stroke="#0F172A" strokeWidth="9" strokeLinecap="round" strokeLinejoin="round"/>
                      <circle cx="32" cy="10" r="6" fill="#020617"/>
                    </svg>
                ...
                <div>
                  <div style={{ color: '#fff', fontSize: 'clamp(16px, 4vw, 56px)', fontWeight: 700, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '2%' }}>
                    İş Süreçlerinizi<br /><span style={{ color: '#F59E0B' }}>AI ile</span> Dönüştürün
                  </div>
                  <div style={{ color: '#78716C', fontSize: 'clamp(9px, 1.5vw, 20px)' }}>Operasyonel mükemmelliği birlikte inşa edelim.</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '2%' }}>
                  <div style={{ background: '#F59E0B', color: '#0F172A', padding: '8px 20px', borderRadius: 10, fontSize: 'clamp(9px, 1.3vw, 16px)', fontWeight: 700 }}>Check-Up Başlatın</div>
                  <span style={{ color: '#4C4462', fontSize: 'clamp(8px, 1vw, 14px)' }}>verimio.com.tr</span>
                </div>
              </div>
              <div style={{ background: '#0F172A', padding: '10px 16px', fontSize: 12, color: '#4C4462' }}>OG Image — 1200×630 (otomatik oluşturuluyor)</div>
            </div>
          </div>
        )}
          ...
          <div>
              ...
              {HAKKIMIZDA_HEROES.slice(0, 3).map((item) => (
                <div key={item.id} style={card()}>
                  <div style={{ aspectRatio: '4/3', overflow: 'hidden', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(10,5,20,0.75)', backdropFilter: 'blur(4px)', borderRadius: 8, padding: '4px 10px', fontSize: 11, fontWeight: 700, color: '#F59E0B' }}>
                      {item.label}
                    </div>
                  </div>
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: '#F59E0B', textDecoration: 'none', fontWeight: 600 }}>↗ Tam boyut</a>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 48 }}>
              {HAKKIMIZDA_HEROES.slice(3).map((item) => (
                <div key={item.id} style={{ ...card(), border: '1px solid #F59E0B' }}>
                  <div style={{ background: '#F59E0B', padding: '6px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#0F172A' }}>YENİ YÖN DENEMESİ</span>
                  </div>
                  <div style={{ aspectRatio: '4/3', overflow: 'hidden', position: 'relative' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={item.url} alt={item.label} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            ...
            {HAKKIMIZDA_HEROES.map((item, i) => (
              <div key={item.id} style={{
                ...
                {/* Sol — metin */}
                <div>
                  <div style={{ fontSize: 11, color: '#F59E0B', fontWeight: 700, letterSpacing: '0.1em', marginBottom: 16 }}>
                    VERİMİO HAKKINDA
                  </div>
                  <div style={{ fontSize: 34, fontWeight: 700, color: '#FFFFFF', lineHeight: 1.2, marginBottom: 16 }}>
                    Operasyonel<br /><span style={{ color: '#F59E0B' }}>mükemmelliği</span><br />birlikte inşa edelim.
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    <div style={{ background: '#F59E0B', color: '#0F172A', padding: '10px 22px', borderRadius: 10, fontSize: 13, fontWeight: 700 }}>
                      Check-Up Başlatın
                    </div>
                ...
                <div style={{ gridColumn: '1/-1', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 12,
                  ...
                  <span style={{ background: '#0F172A', padding: '2px 8px', borderRadius: 4, color: '#F59E0B', fontFamily: 'monospace' }}>
                    {item.label}
                  </span>
                  ...
                  {(['linkedin', 'instagram', 'twitter', 'story'] as SocialAspect[]).map((asp, aspIdx) => (
                    <div key={asp} style={{ marginBottom: 56 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                        ...
                        <div style={{ height: 1, flex: 1, background: '#0F172A' }} />
                      </div>
                          ...
                          <div key={img.id} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            ...
                            {aspIdx === 0 && (
                              <a href={img.url} target="_blank" rel="noopener noreferrer"
                                style={{ fontSize: 11, color: '#F59E0B', textDecoration: 'none', textAlign: 'center' }}>
                                ↗ Ham görsel
                              </a>
                            )}
                          </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 32 }}>
              <div>
                ...
                <p style={{ color: '#4C4462', fontSize: 14, maxWidth: 520 }}>
                  ...
                  FAL_KEY gerekli — <span style={{ color: '#F59E0B', fontFamily: 'monospace' }}>.env.local</span>'de olmalı.
                </p>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 56 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                ...
                <span style={{ background: '#F59E0B', color: '#0F172A', fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 6 }}>FLUX only</span>
                <span style={{ background: '#0F172A', color: '#78716C', fontSize: 10, fontWeight: 600, padding: '3px 10px', borderRadius: 6 }}>Blog A + C hybrid · açık zemin</span>
              </div>
              <p style={{ color: '#4C4462', fontSize: 13, marginBottom: 24 }}>
                ...
                {FLUX_V3_HYBRID.map((item) => (
                  <div key={item.id} style={card()}>
                    <div style={{ aspectRatio: '16/9', overflow: 'hidden', position: 'relative' }}>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={item.url} alt={item.label} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      <div style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(245,158,11,0.9)', color: '#0F172A', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6 }}>FLUX 1.1 Pro</div>
                    </div>
                    <div style={{ padding: '14px 16px' }}>
                      ...
                      <a href={item.url} target="_blank" rel="noopener noreferrer"
                        style={{ fontSize: 12, color: '#F59E0B', textDecoration: 'none', fontWeight: 600 }}>
                        ↗ Tam boyut
                      </a>
                    </div>
                  </div>
              ...
              <div style={{ fontSize: 12, color: '#4C4462', marginBottom: 8 }}>
                → Sosyal kart önizlemesi için <strong style={{ color: '#F59E0B' }}>✦ Blog Sosyal</strong> sekmesine bak
              </div>
            </div>
            ...
            <div style={{ marginBottom: 56 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                ...
                <span style={{ background: '#F59E0B', color: '#0F172A', fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 6 }}>FLUX only</span>
                <span style={{ background: '#0F172A', color: '#78716C', fontSize: 10, fontWeight: 600, padding: '3px 10px', borderRadius: 6 }}>Açık & Temiz · Foto+Overlay · Lime orta</span>
              </div>
              <p style={{ color: '#4C4462', fontSize: 13, marginBottom: 24 }}>
                ...
                {FLUX_V2.filter(x => x.id.startsWith('hero')).map((item) => (
                  <div key={item.id} style={card()}>
                    <div style={{ aspectRatio: '4/3', overflow: 'hidden', position: 'relative' }}>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={item.url} alt={item.label} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      <div style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(245,158,11,0.9)', color: '#0F172A', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6 }}>
                        FLUX 1.1 Pro
                      </div>
                    </div>
                    <div style={{ padding: '14px 16px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          style={{ flex: 1, padding: '7px 0', borderRadius: 8, border: '1px solid #0F172A', cursor: 'pointer', fontSize: 11, fontWeight: 600, background: 'transparent', color: '#78716C' }}
                        >
                          Yeniden Üret
                        </button>
                        <a href={item.url} target="_blank" rel="noopener noreferrer"
                          style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid #0F172A', fontSize: 11, fontWeight: 600, color: '#F59E0B', textDecoration: 'none', background: 'transparent' }}>
                          ↗ Tam boyut
                        </a>
                      </div>
                    </div>
            ...
            <div style={{ borderTop: '1px solid #0F172A', paddingTop: 40, marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 32 }}>
              {(Object.keys(AI_PROMPTS) as AICategory[]).map((cat) => (
                <button
                  ...
                  style={{
                    ...
                    background: aiCategory === cat ? '#F59E0B' : '#0F172A',
                    color: aiCategory === cat ? '#0F172A' : '#78716C',
                  }}
                >
              ...
              <button
                ...
                style={{
                  marginLeft: 'auto', padding: '8px 24px', borderRadius: 10, border: '1px solid #F59E0B',
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 32, flexWrap: 'wrap' }}>
              <div style={{ background: '#0F172A', borderRadius: 8, padding: '8px 16px', display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              {AI_PROMPTS[aiCategory].map((item) => {
                ...
                return (
                  <div key={item.id} style={card()}>
                    ...
                    <div style={{
                      ...
                      {state?.loading && (
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ width: 40, height: 40, border: '3px solid #0F172A', borderTopColor: '#F59E0B', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
                      ...
                      <div style={{
                        ...
                        background: item.model === 'recraft' ? 'rgba(139,92,246,0.85)' : 'rgba(245,158,11,0.85)',
                        color: item.model === 'recraft' ? '#FFFFFF' : '#0F172A',
                    ...
                    {/* Alt bilgi + buton */}
                    <div style={{ padding: '14px 16px', borderTop: '1px solid #0F172A' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          style={{
                            ...
                            fontSize: 12, fontWeight: 600,
                            background: state?.loading ? '#0F172A' : '#F59E0B',
                            color: state?.loading ? '#4C4462' : '#0F172A',
                          }}
                        >
